...
def write(voxel_model, fp):
    ...
    # Write header in bytes format
    fp.write(b'#binvox 1\n')
    fp.write(f'dim {" ".join(map(str, voxel_model.dims))}\n'.encode('ascii'))
    fp.write(f'translate {" ".join(map(str, voxel_model.translate))}\n'.encode('ascii'))
    fp.write(f'scale {voxel_model.scale}\n'.encode('ascii'))
    fp.write(b'data\n')
